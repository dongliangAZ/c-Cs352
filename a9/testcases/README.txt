...
As in assignment 8, each test case is set up as a small 
...
Each of these command files takes four arguments:
   ...
   argument 1:	valgrind option (use '-v' if you want to test whether all
                allocated memory was freed, -q otherwise)
...
the executable file "mymake" that is being tested.
...
If you had wanted to run this command with the same file names but 
to check whether there are any memory leaks), you would run
